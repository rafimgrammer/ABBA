...
function answer(q, v) {
  ...
  c.busy = true; update();
  setTimeout(() => {
    c.log.push({ who: 'ai', kind: 'ack', qid: q.id, text: ackFor(ackId, store, a) });
    const next = currentQ();
    if (next) { const nq = qFor(next, a); c.log.push({ who: 'ai', kind: 'q', qid: nq.id, text: nq.text }); }
...
function undo() {
  ...
  a.suggested = false;   // 제안 목표 플래그가 남아 있으면 되돌린 뒤에도 옛 제안이 따라온다
