    ...
    <>
      ...
      <div className="main fade" style={{ paddingBottom: 100 }}>
        <div className="card">
          ...
          <div className="note"><Icon name="info" size={16} /><div>AI 응답의 숫자는 코드 계산값과 대조해서 다르면 다시 생성해요(본 구현 규칙).</div></div>
        </div>
        <div className="card">
          {SRC.map(([i, t, d, cyc, stt, need], k) => (
            <div className={`src ${k ? '' : 'first'}`} key={t}>
              <b><Icon name={i} size={16} />{t} <span className={`tagx ${stt.startsWith('연동') ? 'plan' : ''}`}>{stt}</span></b>
              <p>{d}</p>
              <div className="meta">
                <span>갱신 {cyc}</span>
