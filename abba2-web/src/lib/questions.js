...
// store.js를 import하지 않는다. 예전에는 S를 직접 읽어서 store ↔ questions 순환 참조가 있었다.
// 지금은 답변 객체(a)를 인자로 받으므로 이 파일은 화면·상태와 무관한 순수 함수다.
import { M, LTV } from './config.js';
import { fmt, won, ymAdd } from './utils.js';
import { ENGINE } from './engine.js';

/** @param a 답변 객체(store의 S.a) */
function qFor(id, a) {
  switch (id) {
