...
async def main():
    async with async_playwright() as p:
        ...
        # 규칙: 외부 의존은 웹폰트뿐(라이브러리·CDN 스크립트 금지). 랜딩이 폰트 3종을 더 쓰면서
        # link가 2개가 됐으므로, 개수 대신 "모든 link가 폰트인가"를 본다.
        links = re.findall(r'<link[^>]*>', src)
        fonts_only = all(('fonts.googleapis.com' in l) or ('pretendard' in l.lower()) for l in links)
        check('외부 의존은 웹폰트뿐', fonts_only and src.count('<script src') == 0 and 'pretendard' in src.lower(),
              [l for l in links if 'fonts.googleapis.com' not in l and 'pretendard' not in l.lower()])
        ...
        body = await pg.inner_text('.main')
        check('데이터 화면 출처 7개', await pg.locator('.src').count() == 7)
        check('데이터 화면에 AI 모델 연결됨 표기', 'gemini' in body.lower() and '연결됨' in body)
        ...
        # ---------- 4-1. AI 상세 계획 (네트워크를 타지 않는 부분만) ----------
        # 자동화 브라우저에서는 자동 호출이 꺼지고 수동 버튼이 뜬다(navigator.webdriver).
        # 실제 Gemini 호출 검증은 tests/test_ai.py에서 따로 한다.
        await pg.click('a[href="#/plan"]'); await pg.wait_for_timeout(250)
        check('AI 카드 표시', await pg.is_visible('text=AI 상세 계획'))
        check('자동화에서는 수동 버튼', await pg.is_visible('button:has-text("AI 상세 계획 만들기")'))
        check('AI 카드에 근거 고지 표시', 'AI는 설명 문장만' in await pg.inner_text('.main'))
        if TARGET == 'dist':
            # 숫자 검증기: 확정 수치에 있는 금액은 통과, 없는 금액은 잡아낸다.
            v = await pg.evaluate("""() => {
              const risk = S.planRisk || S.a.risk;
              const facts = COACH.factSheet(S.a, planCalc(S.a, risk), risk);
              const allow = COACH.allowList(facts);
              return {
                clean: COACH.verify({ a: '예상 총액은 2,248만 원이에요' }, allow),
                dirty: COACH.verify({ a: '예상 총액은 9,999만 원이에요' }, allow),
                nested: COACH.verify({ s: [{ d: '매달 31.5만 원' }, { d: '매달 77.7만 원' }] }, allow),
                inFacts: facts.includes('2,248만 원'),
              };
            }""")
            check('확정 수치의 금액은 검증 통과', v['clean'] == [], v['clean'])
            check('없는 금액은 검증에서 걸림', v['dirty'] == ['9999만원'], v['dirty'])
            check('중첩 구조도 검사', v['nested'] == ['77.7만원'], v['nested'])
            check('팩트시트에 엔진 계산값 포함', v['inFacts'])

            # 표현 검사: 보장·단정·매매 권유는 걸리고, 부정형·정상 표현은 통과해야 한다
            ph = await pg.evaluate("""() => ({
              보장:    COACH.checkPhrases({ x: '이 계획은 수익을 보장합니다' }),
              부정형:  COACH.checkPhrases({ x: '수익을 보장하지 않습니다' }),
              권유:    COACH.checkPhrases({ x: '반드시 오르니 지금 매수하세요' }),
              정상:    COACH.checkPhrases({ x: '국내 지수 ETF를 적립 매수합니다' }),
              위험부인: COACH.checkPhrases({ x: '손해 볼 일 없어요' }),
            })""")
            check('수익 보장 표현 차단', ph['보장'] == ['수익·원금 보장'], ph['보장'])
            check('부정형은 통과(수익을 보장하지 않습니다)', ph['부정형'] == [], ph['부정형'])
            check('단정·매매 권유 차단', len(ph['권유']) >= 1, ph['권유'])
            check('정상 표현은 통과(적립 매수)', ph['정상'] == [], ph['정상'])
            check('위험 부인 표현 차단', ph['위험부인'] == ['위험 부인'], ph['위험부인'])

            # 템플릿 폴백: 정의상 숫자·표현 검증을 통과해야 한다(통과 못하면 폴백이 무의미)
            tp = await pg.evaluate("""() => {
              const risk = S.planRisk || S.a.risk;
              const c = planCalc(S.a, risk);
              const t = COACH.templatePlan(S.a, c, risk);
              return { num: COACH.verify(t, COACH.allowList(COACH.factSheet(S.a, c, risk))),
                       phrase: COACH.checkPhrases(t), steps: t.steps.length, miles: t.milestones.length };
            }""")
            check('템플릿 폴백 숫자 위반 없음', tp['num'] == [], tp['num'])
        ...
        check('설정에 키 보관 위치 안내', '저장소에 올라가지 않아요' in await pg.inner_text('.main'))
        await pg.click('button.sw[aria-label="ai"]'); await pg.wait_for_timeout(150)
        await pg.click('a[href="#/plan"]'); await pg.wait_for_timeout(250)
        check('AI 끄면 카드 숨김', not await pg.is_visible('text=AI 상세 계획'))
        await pg.click('a[href="#/settings"]'); await pg.wait_for_timeout(200)
        await pg.click('button.sw[aria-label="ai"]'); await pg.wait_for_timeout(150)  # 원복
